...
# title: "energyUsePerFuelYeartab.Rmd"
# author: "Eric Koski"
# date: "12/16/2019"
...
GFLenergyPerYearFueltab <- GFLcountyEnergyPerFuelYear_3dig %>% 
  group_by(YEAR) %>% 
...
earliestRow <- filter(GFLenergyPerYearFueltab, Year == min(Year))
latestRow <- filter(GFLenergyPerYearFueltab, Year == max(Year))
...
GFLenergyPerYearFueltab <- GFLenergyPerYearFueltab %>%
  mutate_if(is.numeric, ~formatC(as.integer(round(.)), big.mark = ","))

names(GFLenergyPerYearFueltab) <- 
  str_replace_all(names(GFLenergyPerYearFueltab), "_", " ")

# Render the table
if (outputFormat == "pdf_document") {
  GFLenergyPerYearFueltab %>%
    kable(escape = FALSE, digits = 0, booktabs = TRUE,
          align = c("crrrrrrrrr"),
          linesep = c(""),
          caption = tableCaption(
            "Annual industrial and agricultural energy use (millions of BTU) by fuel type")) %>%
    row_spec(length(GFLenergyPerYearFueltab$Year) - 1, hline_after = TRUE) %>%
