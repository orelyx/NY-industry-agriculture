...
leftTicks <- axisTicks(c(0, 1.17 * max(GHGemissionsAgDetailHighest$CO2e100mt)), 
                       log = FALSE, nint = 5)
...
GHGAgricultureforplot <- GHGemissionsAgDetailHighest %>%
  mutate(secondAxisVals = CO2eCumFract * max(leftTicks))

xlabels <- as.character(GHGAgricultureforplot$NAICS4dig[
                       seq(1:length(GHGAgricultureforplot$NAICS4dig))])
xlabels[length(GHGAgricultureforplot$NAICS4dig)] <- "rest"

GHGAgricultureplot <- ggplot(GHGAgricultureforplot) +
  scale_color_manual(values = c("#2f4f4f"), name = "") +
  scale_fill_manual(values = c("#3cb44b"), name = "") +
  ...
  scale_x_continuous(breaks = seq(0, nrow(GHGAgricultureforplot)-1, by=1), 
                     minor_breaks = NULL,
                     labels = xlabels) +
  ...
  geom_col(aes(seq(0, nrow(GHGAgricultureforplot)-1, by=1), 
               CO2e100mt, fill = "emissions")) +
  geom_line(aes(seq(0, nrow(GHGAgricultureforplot)-1, by=1), 
                secondAxisVals, color = "percentage")) +
  guides(color = guide_legend(order = 1), 
         fill = guide_legend(order = 2))
  
  
show(GHGAgricultureplot)
