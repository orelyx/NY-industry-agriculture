```{r }
# title: "countySectorYearFueltab.Rmd"
# author: "Eric Koski"
# date: "12/11/2019"
...
GFLcountySectorYearFuel_tab <- select(GFLcountyEnergyPerFuelYear_1dig, -NAICS1dig)

# We want the initial columns ordered County, Sector, Year, then fuels in decreasing 
# order of total use. 
GFLcountySectorYearFuel_tab <- GFLcountySectorYearFuel_tab %>% 
  rename(Year = YEAR) %>%
...
names(GFLcountySectorYearFuel_tab) <- 
  str_replace_all(names(GFLcountySectorYearFuel_tab), "_", " ")

# How precisely we render the table depends on whether we're rendering to HTML or pdf. 
if (outputFormat == "pdf_document") {
  GFLcountySectorYearFuel_tab %>%
