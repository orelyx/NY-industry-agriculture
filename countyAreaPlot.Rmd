...
# Make our own copy of GFLenergyPerCountyYearFuel with columns added 
# to represent upper and lower bounds for area plots. 
# Note use of as.symbol(), "!!", and ":=" allowing use of 
# synthesized column names. 
GFLenergyPerCountyYearFuelBounds <- ungroup(GFLenergyPerCountyYearFuel)

for (fuel in fuelNames) {
  if (fuel == fuelNames[1])
  {
    name1 <- as.symbol(str_c(fuel, "Lower"))
    GFLenergyPerCountyYearFuelBounds <- GFLenergyPerCountyYearFuelBounds %>%
      mutate(!!name1 := seq(0, 0, length.out = length(GFLenergyPerCountyYearFuelBounds[[fuel]])))
    lastFuel <- fuel
  } else {
    name1 <- as.symbol(str_c(fuel, "Lower"))
    name2 <- str_c(lastFuel, "Upper")
    GFLenergyPerCountyYearFuelBounds <- GFLenergyPerCountyYearFuelBounds %>%
      mutate(!!name1 := GFLenergyPerCountyYearFuelBounds[[name2]])
  }
  name1 <- as.symbol(str_c(fuel, "Upper"))
  name2 <- str_c(fuel, "Lower")
  GFLenergyPerCountyYearFuelBounds <- GFLenergyPerCountyYearFuelBounds %>%
    mutate(!!name1 := GFLenergyPerCountyYearFuelBounds[[name2]] + 
             GFLenergyPerCountyYearFuelBounds[[fuel]])
  lastFuel <- fuel
}
...
countiesAreaPlot <- ggplot(GFLenergyPerCountyYearFuelBounds) +
