...
GFLenergyPerNonMSector3dYearFuel <- GFLcountyEnergyPerFuelYear_3dig %>% 
...
# Make our own copy of GFLenergyPerNonMSector3dYearFuel with columns added 
# to represent upper and lower bounds for area plots. 
GFLenergyPerNonMSector3dYearFuelBounds <- ungroup(GFLenergyPerNonMSector3dYearFuel) 

for (fuel in fuelNames) {
  if (fuel == fuelNames[1])
  {
    name1 <- as.symbol(str_c(fuel, "Lower"))
    GFLenergyPerNonMSector3dYearFuelBounds <- 
      GFLenergyPerNonMSector3dYearFuelBounds %>%
      mutate(!!name1 := seq(0, 0, length.out = 
                              length(GFLenergyPerNonMSector3dYearFuelBounds[[fuel]])))
    lastFuel <- fuel
  } else {
    name1 <- as.symbol(str_c(fuel, "Lower"))
    name2 <- str_c(lastFuel, "Upper")
    GFLenergyPerNonMSector3dYearFuelBounds <- GFLenergyPerNonMSector3dYearFuelBounds %>%
      mutate(!!name1 := GFLenergyPerNonMSector3dYearFuelBounds[[name2]])
  }
  name1 <- as.symbol(str_c(fuel, "Upper"))
  name2 <- str_c(fuel, "Lower")
  GFLenergyPerNonMSector3dYearFuelBounds <- GFLenergyPerNonMSector3dYearFuelBounds %>%
    mutate(!!name1 := GFLenergyPerNonMSector3dYearFuelBounds[[name2]] + 
             GFLenergyPerNonMSector3dYearFuelBounds[[fuel]])
  lastFuel <- fuel
}
...
regionsNonM3dAreaPlot <- ggplot(GFLenergyPerNonMSector3dYearFuelBounds) +
