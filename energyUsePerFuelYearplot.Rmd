...
GFLenergyPerYearFuel <- GFLcountyEnergyPerFuelYear_3dig %>% 
  group_by(YEAR) %>% 
  summarize_at(vars(all_of(fuelNames)), sum, na.rm = TRUE) 

# Make our own copy of GFLenergyPerYearFuel with columns added 
# to represent upper and lower bounds for area plots. 
GFLenergyPerYearFuelBounds <- ungroup(GFLenergyPerYearFuel) 

for (fuel in fuelNames) {
  if (fuel == fuelNames[1])
  {
    name1 <- as.symbol(str_c(fuel, "Lower"))
    GFLenergyPerYearFuelBounds <- 
      GFLenergyPerYearFuelBounds %>%
      mutate(!!name1 := seq(0, 0, length.out = 
                              length(GFLenergyPerYearFuelBounds[[fuel]])))
    lastFuel <- fuel
  } else {
    name1 <- as.symbol(str_c(fuel, "Lower"))
    name2 <- str_c(lastFuel, "Upper")
    GFLenergyPerYearFuelBounds <- GFLenergyPerYearFuelBounds %>%
      mutate(!!name1 := GFLenergyPerYearFuelBounds[[name2]])
  }
  name1 <- as.symbol(str_c(fuel, "Upper"))
  name2 <- str_c(fuel, "Lower")
  GFLenergyPerYearFuelBounds <- GFLenergyPerYearFuelBounds %>%
    mutate(!!name1 := GFLenergyPerYearFuelBounds[[name2]] + 
             GFLenergyPerYearFuelBounds[[fuel]])
  lastFuel <- fuel
}
...
energyPerYearFuelPlot <- ggplot(GFLenergyPerYearFuelBounds) +
