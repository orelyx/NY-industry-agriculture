...
GFLcountySectorLYFuel_tab <- GFLcountyEnergyPerFuelLY_4dig

# We want the initial columns ordered County, Sector
GFLcountySectorLYFuel_tab <- GFLcountySectorLYFuel_tab %>% 
...
names(GFLcountySectorLYFuel_tab) <- 
  str_replace_all(names(GFLcountySectorLYFuel_tab), "_", " ")

write_tsv(GFLcountySectorLYFuel_tab, "GFLcountySectorLYFuel_tab.csv")

aList <- NULL
countyList <- 
  if (TRUE) {
    for (county in unique(GFLcountySectorLYFuel_tab$County)) {
      aList[[county]] <- sum(GFLcountySectorLYFuel_tab$County == county)
    }
    aList
  }
# print(countyList)
...
if (outputFormat == "pdf_document") {
  GFLcountySectorLYFuel_tab %>%
    kable(escape = FALSE, digits = 0, longtable = TRUE,
          align = c("lrrrrrrrr"), 
          caption = tableCaption(str_c(
            as.character(max(GFLcountyEnergyPerFuelLY_4dig$YEAR)),
